...
### Treasurer Dues: Fix remove-students allocation cleanup
- **Title:** Corrected deletion of payment allocations when removing students from a due
- **Details:**
  - Fixed backend to delete from `payment_allocations_dues` via `user_dues` IDs instead of non-existent `payment_allocations` by `due_id`/`user_id`.
  - Ensures referential cleanup before removing `user_dues` rows to prevent FK violations and orphaned allocations.
- **Files Updated:** `backend/src/controllers/treasurerController.js` (`removeStudentsFromDue`)
- **Impact:** Stable student removal from dues; no more server errors during bulk removals.
