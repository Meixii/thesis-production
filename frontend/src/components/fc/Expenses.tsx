...
const Expenses: React.FC = () => {
  ...
  // Fetch member payment status for an expense
  const handleViewMemberStatus = async (expense: Expense) => {
    ...
    try {
      ...
      const groupId = profile.groupId;
      if (!groupId) throw new Error('No group assigned');
      const res = await fetch(getApiUrl(`/api/groups/${groupId}/expenses/${expense.id}/members-status`), {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!res.ok) throw new Error('Failed to fetch member payment status');
      const data = await res.json();
      setMemberStatus(data.data);
    } catch (err) {
      setMemberStatusError(err instanceof Error ? err.message : 'Unknown error');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  ...
                  {expenses.map((expense) => (
                    <tr key={expense.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-700/50 transition-colors duration-150">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-neutral-900 dark:text-white">
                        ...
                        <button
                          onClick={() => handleViewMemberStatus(expense)}
        ...
        {/* Member Status Modal (only show if not viewing receipt) */}
        {viewingExpense && !viewingReceiptUrl && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-40">
            <Card className="w-full max-w-5xl max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold">Member Payment Status for: <span className="font-bold">{viewingExpense.description}</span></h3>
                  <button
                    onClick={() => { setViewingExpense(null); setMemberStatus(null); setMemberStatusError(null); }}
                    className="text-neutral-500 hover:text-neutral-700 dark:text-neutral-400 dark:hover:text-neutral-200"
                    disabled={!!viewingReceiptUrl}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                ...
                {memberStatusLoading ? (
                  <div className="flex flex-col items-center justify-center py-8">
                    ...
                    <p className="text-neutral-600 dark:text-neutral-400">Loading member status...</p>
                  </div>
                ) : memberStatusError ? (
                  <div className="bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-400 p-4 rounded-md">
                    <p className="font-semibold">Error loading member status</p>
                    <p className="text-sm mt-1">{memberStatusError}</p>
                  </div>
                ) : memberStatus && memberStatus.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700 text-sm">
                      <thead>
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Email</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Role</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Status</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Method</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Ref ID</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Receipt</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-neutral-200 dark:divide-neutral-700">
                        {memberStatus.map((member) => (
                          <tr key={member.user_id}>
                            <td className="px-4 py-2 whitespace-nowrap font-medium text-neutral-900 dark:text-white">{member.name}</td>
                            <td className="px-4 py-2 whitespace-nowrap text-neutral-700 dark:text-neutral-300">{member.email}</td>
                            <td className="px-4 py-2 whitespace-nowrap text-neutral-700 dark:text-neutral-300">{member.role === 'finance_coordinator' ? 'Finance Coordinator' : 'Student'}</td>
                            <td className="px-4 py-2 whitespace-nowrap">
                              <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                                ${member.status === 'paid' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300'
                                  : member.status === 'pending_verification' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300'
                                  : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300'
                                }`}>
                                {member.status === 'paid' ? 'Paid' : member.status === 'pending_verification' ? 'Pending Verification' : 'Not Paid'}
                              </span>
                            </td>
                            <td className="px-4 py-2 whitespace-nowrap text-neutral-700 dark:text-neutral-300">{member.method || 'N/A'}</td>
                            <td className="px-4 py-2 whitespace-nowrap text-neutral-700 dark:text-neutral-300">{member.reference_id || 'N/A'}</td>
                            <td className="px-4 py-2 whitespace-nowrap">
                              {member.receipt_url ? (
                                <button
                  ...
                  <div className="text-neutral-500 dark:text-neutral-400">No members found.</div>
                )}
              </div>
            </Card>
          </div>
        ...
        {/* Receipt Image Modal (only show if viewingReceiptUrl is set) */}
        {viewingReceiptUrl && (
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-60">
            <div className="bg-white dark:bg-neutral-900 rounded-lg shadow-lg p-4 max-w-2xl w-full flex flex-col items-center">
